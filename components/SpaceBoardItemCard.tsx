...
function getBoardDisplayLabel(sticky: GroupSticky): string {
  if (sticky.displayMode === 'alert') return 'Alert';
  if (sticky.displayMode === 'announcement') return 'Announcement';

  return 'Pinned note';
}

export default function SpaceBoardItemCard({
  sticky,
  theme,
  isAdmin,
  authorLabel,
    ...
    <View style={s.stickyCard}>
      <View style={s.stickyTop}>
        <View style={{ flex: 1 }}>
          ...
          <Text style={s.legacyStickyLabel}>{getBoardDisplayLabel(sticky)}</Text>
